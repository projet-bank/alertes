...
function processExcelFile(file) {
  ...
  reader.onload=function(e){
    try {
      ...
      // C'est la première ligne qui contient "nature" ou "document" ou "titre"
      const keywords=['nature','document','titre','libelle','rapport'];
      let headerRowIdx=-1;
      for(let i=0;i<Math.min(rows.length,15);i++){
        const rowStr=rows[i].map(c=>stripAccents(c)).join(' ');
        if(keywords.some(k=>rowStr.includes(k))){headerRowIdx=i;break;}
      }
      if(headerRowIdx===-1){showCsvStatus('error','❌ Impossible de trouver la ligne d\'en-tête (colonne "Nature" introuvable dans les 15 premières lignes).');return;}
      ...
      if(iDeadline===-1){showCsvStatus('error','❌ Colonne "Date limite / Délai" introuvable. En-têtes détectés : '+headers.filter(h=>h).join(' | '));return;}
      ...
      for(let i=0;i<dataRows.length;i++){
        ...
        if(!rowFlat)continue;
        if(keywords.some(k=>stripAccents(row[iTitle]||'').includes(k)&&String(row[iTitle]).length<30))continue;
        ...
        const dl=normalizeDate(String(row[iDeadline]||'').trim());
        const p=iPeriode>=0?String(row[iPeriode]||'').trim()||'Non défini':'Non défini';
        const e=(lastEntity||'AUTRE').toUpperCase();

        if(!t){skipped.push('Ligne '+(headerRowIdx+i+2)+' : titre manquant');continue;}
        if(!dl||!pDate(dl)){skipped.push('Ligne '+(headerRowIdx+i+2)+' : date limite invalide ("'+String(row[iDeadline]||'')+'")'  );continue;}
