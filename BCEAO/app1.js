...
function renderCsvDataTable(){
  ...
  const urgCount=csvData.filter(d=>{const x=dUntil(nextNotifDate(d.a,d.p));return x!==null&&x>=0&&x<=settings.urgentDays;}).length;
  const warnCount=csvData.filter(d=>{const x=dUntil(nextNotifDate(d.a,d.p));return x!==null&&x>settings.urgentDays&&x<=settings.warnDays;}).length;
  const pastCount=0; // Avec la nouvelle logique, les notifs cycliques ne sont jamais "passées"
  const okCount=csvData.filter(d=>{const x=dUntil(nextNotifDate(d.a,d.p));return x!==null&&x>settings.warnDays;}).length;
  ...
  tbody.innerHTML=csvData.slice(0,50).map(d=>{const key=d.t+'|'+d.a,next=nextNotifDate(d.a,d.p),nextStr=next?next.toISOString().slice(0,10):'—',days=dUntil(next),st=getSt(days),isTrans=transmittedSet.has(key);return`<tr style="${isTrans?'opacity:0.55':''}"><td><span class="status-dot ${st}"></span>${isTrans?' <span style="font-size:10px;color:var(--green)">✅</span>':''}</td><td class="td-title">${d.t}</td><td><span class="entity-pill" style="color:${eC(d.e)};background:${eCb(d.e)}">${d.e}</span></td><td style="font-size:12px;color:var(--text3)">${d.p}</td><td style="font-family:'JetBrains Mono',monospace;font-size:12px">${fmtS(d.a)}</td><td style="font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:600">${fmtS(nextStr)}</td><td><span class="badge ${st}">${dLabel(days,st)}</span></td></tr>`;}).join('')+(csvData.length>50?`<tr><td colspan="7" style="text-align:center;color:var(--text3);font-size:12px;padding:12px">… et ${csvData.length-50} autres</td></tr>`:'');
}
...
// Calcule la prochaine date de notification à partir de la date d'arrêté et de la périodicité
function periodDays(period){
  const map={'Quotidienne':1,'Hebdomadaire':7,'Bimensuelle':15,'Mensuelle':30,'Bimestrielle':60,'Trimestrielle':90,'Semestrielle':180,'Annuelle':365};
  return map[period]||30;
}
function nextNotifDate(arreteStr, period){
  const base=pDate(arreteStr);
  if(!base)return null;
  const days=periodDays(period);
  // Avancer par pas de périodicité jusqu'à trouver la prochaine date >= aujourd'hui
  let next=new Date(base);
  // Limite de sécurité : max 20 ans d'avance
  let iterations=0;
...
function buildData(){
  ...
  ALL=raw.map((d,i)=>{
    ...
    // Calculer la prochaine date de notification via périodicité
    const nextD=nextNotifDate(arrStr,d.p);
    const days=dUntil(nextD);
    const key=d.t+'|'+(arrStr||d.t);
    const transmitted=transmittedSet.has(key);
    // Si transmis, calculer la prochaine occurrence après transmission
    let dlD=nextD,finalDays=days,st;
    if(transmitted){
      // Relancer : chercher la prochaine date APRÈS aujourd'hui
      if(nextD&&nextD<=TODAY){
        let next2=new Date(nextD);
        const daysP=periodDays(d.p);
        let it=0;
        while(next2<=TODAY&&it<10000){next2=new Date(next2.getTime()+daysP*86400000);it++;}
        dlD=next2;finalDays=dUntil(next2);
        // Retirer du set transmis car nouvelle période a démarré
        transmittedSet.delete(key);
      } else {
        st='past';
      }
    }
    st=st||getSt(finalDays);
    // Convertir dlD en string ISO pour affichage
    const dlStr=dlD?dlD.toISOString().slice(0,10):'—';
    return{id:i,key,title:d.t,arrete:arrStr||'—',deadline:dlStr,period:d.p,entity:d.e,dlD,days:finalDays,st,custom:i>=csvData.length,transmitted:transmittedSet.has(key)};
...
async function markTransmittedById(docKey){
  ...
  // Recalculer : buildData va avancer la prochaine notification automatiquement
  ...
  showToast('✅','Document transmis — prochaine notification planifiée');
}
...
function updateDocPeriod(docId, newPeriod){
  const d=ALL.find(x=>x.id===docId);if(!d)return;
  // Mettre à jour dans csvData ou customDL
  if(d.custom){const ci=docId-csvData.length;if(customDL[ci])customDL[ci].p=newPeriod;}
  else{if(csvData[docId])csvData[docId].p=newPeriod;}
  saveToFirestore();buildData();updateKPIs();renderTable();showToast('✓','Périodicité mise à jour : '+newPeriod);
}
...
function renderCustomList(){const tbody=document.getElementById('custom-tbody'),emp=document.getElementById('custom-empty');if(customDL.length===0){tbody.innerHTML='';emp.style.display='';return;}emp.style.display='none';tbody.innerHTML=customDL.map((d,i)=>{const next=nextNotifDate(d.a,d.p);const nextStr=next?next.toISOString().slice(0,10):'—';return`<tr><td class="td-title">${d.t}</td><td><span class="entity-pill" style="color:${eC(d.e)};background:${eCb(d.e)}">${d.e}</span></td><td style="font-family:'JetBrains Mono',monospace;font-size:12px">${fmtS(d.a)}</td><td style="font-size:12px;color:var(--text3)">${d.p}</td><td style="font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:600">${fmtS(nextStr)}</td><td><button class="btn-delete" onclick="delCustom(${i})">Supprimer</button></td></tr>`;}).join('');}
...
function addFromModal(){const t=document.getElementById('am-title').value.trim(),a=document.getElementById('am-arrete').value;if(!t||!a){showToast('⚠\uFE0F','Titre et date d\'arrêté requis');return;}customDL.push({t,a,p:document.getElementById('am-period').value,e:document.getElementById('am-entity').value});saveToFirestore();buildData();updateKPIs();closeOverlay('add-overlay');document.getElementById('am-title').value=document.getElementById('am-arrete').value='';showToast('\u2713','"'+t+'" ajouté');}
function addFromForm(){const t=document.getElementById('f-title').value.trim(),a=document.getElementById('f-arrete').value;if(!t||!a){showToast('⚠️','Titre et date d\'arrêté requis');return;}customDL.push({t,a,p:document.getElementById('f-period').value,e:document.getElementById('f-entity').value});saveToFirestore();buildData();updateKPIs();renderCustomList();document.getElementById('f-title').value=document.getElementById('f-arrete').value='';showToast('✓','"'+t+'" ajouté');}
