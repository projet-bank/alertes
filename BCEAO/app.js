...
function buildData(){
  ...
  ALL=raw.map((d,i)=>{
    ...
    const joursVal=d.jours||periodDays(d.p);

    // Calculer la prochaine date de notification (sans tenir compte de "transmis")
    const nextD=nextNotifDate(arrStr,d.p,joursVal);

    // Si transmis : calculer la date de la PÉRIODE SUIVANTE après nextD
    // Le document reste "passé/transmis" jusqu'à ce que cette date future arrive
    let dlD, finalDays, st, nextDeadline;
    if(transmitted){
      // La période suivante = nextD + durée d'une période
      const daysP=periodDays(d.p,joursVal);
      nextDeadline=nextD?new Date(nextD.getTime()+daysP*86400000):null;
      // Si cette prochaine période est déjà dans le passé (cas de vieilles données), avancer encore
      if(nextDeadline){
        let it=0;
        while(nextDeadline<TODAY&&it<5000){nextDeadline=new Date(nextDeadline.getTime()+daysP*86400000);it++;}
      }
      dlD=nextDeadline;
      finalDays=dUntil(dlD);
      // Le document reste "past" TANT QUE la prochaine date n'est pas encore dans la fenêtre de rappel
      // Dès qu'elle entre dans la fenêtre urgente/proche, on retire automatiquement du set transmis
      const newSt=getSt(finalDays);
      if(newSt==='urgent'||newSt==='warn'||newSt==='ok'){
        // La nouvelle période est arrivée : retirer du set transmis, le doc redevient actif
        transmittedSet.delete(key);
        st=newSt;
      } else {
        // Encore loin : garder comme "past" (transmis en attente)
        st='past';
      }
    } else {
      dlD=nextD;
      finalDays=dUntil(nextD);
      st=getSt(finalDays);
    }

    const dlStr=dlD?dlD.toISOString().slice(0,10):'—';
    return{
      id:i, key, title:d.t, arrete:arrStr||'—', deadline:dlStr,
      period:d.p, jours:joursVal, entity:d.e,
      dlD, days:finalDays, st,
      custom:i>=csvData.length,
      transmitted:transmittedSet.has(key)
    };
...
function transmitBtn(d,small=false){
  ...
  if(d.transmitted){
    return`<div style="display:flex;flex-direction:column;gap:4px;align-items:flex-start">
      <span class="transmitted-badge">✅ Transmis</span>
      <button class="btn-cancel-transmit" onclick="event.stopPropagation();unmarkTransmittedById('${k}')">↩ Annuler</button>
    </div>`;
  }
...
function updateDocJours(docId, newJours){
  ...
  const d=ALL.find(x=>x.id===docId);if(!d)return;
  // 1. Mettre à jour la source primaire (csvData ou customDL)
  if(d.custom){
    const ci=docId-csvData.length;
    if(customDL[ci])customDL[ci].jours=j;
  } else {
    if(csvData[docId])csvData[docId].jours=j;
  }
  // 2. Rebuild complet depuis la source mise à jour
  saveToFirestore();
  buildData();
  updateKPIs();
  // 3. Re-rendre sans appeler renderTable() qui recrée les inputs (évite la perte de focus)
  // On met à jour directement la cellule de la prochaine notification dans la ligne concernée
  renderTable();
...
function renderTimeline(){
  ...
  // Remplir les sélecteurs dynamiquement si pas encore fait
  const yearSel=document.getElementById('cal-filter-year');
  const monthSel=document.getElementById('cal-filter-month');
