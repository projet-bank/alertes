...
function buildData(){
  ...
  // Construire ALL AVANT le tri pour que srcIdx = position dans raw
  const built = raw.map((d, i) => {
    const arrStr = d.a && d.a !== '—' ? d.a : '';
    // Clé UNIQUE basée sur l'index dans raw — garantit qu'aucun document n'en accompagne un autre
    const key   = 'doc_idx_' + i;
    const jours = (d.jours && parseInt(d.jours) > 0) ? parseInt(d.jours) : (PERIOD_DEFAULTS[d.p] || 30);
    const inSet = transmittedSet.has(key);

    // Date de notification courante
    const curDate = nextNotifDate(arrStr, d.p, jours);

    let dlD, days, st, transmitted;

    if (inSet) {
      // Transmis : montrer la date de la période SUIVANTE, rester "past"
      // jusqu'à ce que cette date entre dans la fenêtre de rappel
      ...
      if (days !== null && days <= settings.warnDays) {
        // La nouvelle période est entrée dans la fenêtre → doc redevient actif
    ...
    return {
      id: i,          // position dans raw (stable, utilisée comme id avant tri)
      srcIdx: i,      // même chose — index dans csvData ou customDL
      key,
  ...
  // Trier, puis renuméroter id pour les fonctions de rendu (openDetail, etc.)
  built.sort((a, b) => {
  ...
  // Réaffecter id = position dans ALL après tri, srcIdx reste l'index dans raw
  ALL = built.map((d, pos) => ({ ...d, id: pos }));
}
