/* ═══════════════════════════════════════════════════════
   ...
   Firebase · Excel · Cloud Functions · Notifications · Transmis
═══════════════════════════════════════════════════════ */
'use strict';
...
function downloadExcelTemplate() {
  ...
  // Template basé sur le fichier BCEAO officiel
  const data=[
    ["Nature du document","Date d'arrêté","Date limite","Périodicité","Entité","Correspondants internes"],
    // ── COMPTA ──
    ["États de synthèse périodiques du PCB révisé","2025-01-01","2025-02-15","Mensuelle","COMPTA",""],
    ["États de synthèse périodiques du PCB révisé","","2025-04-15","Trimestrielle","COMPTA"],
    ["États de synthèse périodiques du PCB révisé","","2026-01-15","Annuelle","COMPTA"],
    ["Formulaire de Déclaration Prudentielle (FODEP)","2025-06-01","2025-10-31","Semestrielle","COMPTA"],
    // ── RISQUE ──
    ["État de déclaration des expositions (exemption emprunteurs souverains)","2025-06-01","2025-09-30","Semestrielle","RISQUE"],
    // ── CONTRÔLE DE GESTION ──
    ["FODEP Infra-semestriel","2025-03-31","2025-04-30","Mensuelle","CONTRÔLE DE GESTION"],
    ["FODEP Infra-semestriel","","2025-04-30","Trimestrielle","CONTRÔLE DE GESTION"],
    ["Dispositif provisoire de suivi de la liquidation (DEC 2062 et 2063)","2025-03-31","2025-04-30","Mensuelle","CONTRÔLE DE GESTION"],
    ["Rapport sur la situation financière","2025-03-31","2025-04-30","Trimestrielle","CONTRÔLE DE GESTION"],
    ["Reporting FGDR-UMOA","2025-03-31","2025-04-15","Trimestrielle","CONTRÔLE DE GESTION"],
    ["État financiers individuels annuels certifiés","2024-12-31","2025-06-30","Annuelle","CONTRÔLE DE GESTION"],
    // ── AUDIT ──
    ["Rapport semestriel sur le contrôle interne","2025-06-30","2025-08-31","Semestrielle","AUDIT"],
    // ── DRC ──
    ["Rapport sur la révision semestrielle du portefeuille","2025-06-30","2025-08-31","Semestrielle","DRC"],
    ["Rapport annuel sur le dispositif global de gestion des risques","2024-12-31","2025-04-30","Annuelle","DRC"],
    // ── CONFORMITE ──
    ["Rapport semestriel sur l'évaluation du risque de non-conformité","2025-06-30","2025-07-31","Semestrielle","CONFORMITE"],
    // ── D.FINANCIERE ──
    ["Lettre de mission des Commissaires aux comptes","Au titre de l'année 2025","2026-01-31","Annuelle","D.FINANCIERE"],
    // ── DJRC ──
    ["Liste des Dirigeants (art. 11 circ. 02-2017)","2025-01-01","2025-01-01","Semestrielle","DJRC"],
    ["État des conditions de banque","2025-06-30","2025-07-05","Semestrielle","DJRC"],
    ["Annuaire des Établissements de Crédit","2024-12-31","2026-01-15","Annuelle","DJRC"],
    // ── MARKETING ──
    ["Publication des conditions de banque dans la presse","2025-01-01","2025-01-31","Semestrielle","MARKETING"],
    ["Dispositif de suivi du taux de bancarisation","2024-12-31","2025-10-15","Trimestrielle","MARKETING"],
    // ── ENGAGEMENTS ──
    ["État de suivi du traitement du stock de créances","Le 15 et le 30 de chaque mois","2025-01-15","Bimensuelle","ENGAGEMENTS"],
    // ── TRESORERIE ──
    ["État du relevé quotidien de la position extérieure des banques","Tous les jours","Quotidien","Quotidienne","TRESORERIE"],
    ["État de calcul des besoins courants et état MCCE","Tous les jours","Quotidien","Quotidienne","TRESORERIE"],
    ["Etat des variations prévisionnelles des facteurs autonomes","Tous les lundis","Hebdomadaire","Hebdomadaire","TRESORERIE"],
    ["État des opérations interbancaires","Tous les lundis","Hebdomadaire","Hebdomadaire","TRESORERIE"]
  ].filter(r=>!String(r[0]).startsWith('//'));
  const ws=XLSX.utils.aoa_to_sheet(data);
  ws['!cols']=[{wch:55},{wch:18},{wch:15},{wch:22},{wch:25},{wch:30}];
  XLSX.utils.book_append_sheet(wb,ws,"Échéances BCEAO");
  XLSX.writeFile(wb,"modele_bceao_regalert.xlsx");
  showToast('📥','Modèle BCEAO téléchargé');
}


/* ══ Cloud Function trigger ══════════════════════════════ */
async function triggerCloudEmailNow() {
  const btn=document.getElementById('cf-trigger-btn');
  const status=document.getElementById('cf-status');
  if(!currentUser){showCfStatus('error','⚠️ Connectez-vous d\'abord.');return;}
  if(btn){btn.disabled=true;btn.textContent='⏳ Envoi en cours…';}
  showCfStatus('loading','⏳ Déclenchement de la Cloud Function…');
  try{
    const idToken=await currentUser.getIdToken();
    const res=await fetch('https://us-central1-app-bceao.cloudfunctions.net/sendAlertsManual',
      {method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+idToken},
       body:JSON.stringify({uid:currentUser.uid})});
    const data=await res.json();
    if(res.ok&&data.success){showCfStatus('success','✅ '+(data.message||'Emails envoyés.'));showToast('📧','Notifications envoyées !');}
    else{showCfStatus('error','❌ '+(data.error||data.message||'Erreur inconnue.'));}
  }catch(e){showCfStatus('error','❌ Erreur réseau : '+e.message);}
  finally{if(btn){btn.disabled=false;btn.textContent='📧 Envoyer maintenant';}}
}
function showCfStatus(type,msg){
  const el=document.getElementById('cf-status');if(!el)return;
  el.style.display='';
  const c={success:['var(--green-lt)','var(--green)','rgba(26,138,74,0.2)'],error:['var(--red-lt)','var(--red)','rgba(217,48,37,0.2)'],loading:['var(--blue-lt)','var(--blue)','rgba(26,106,191,0.2)']};
  const[bg,col,bd]=c[type]||c.loading;
  el.style.cssText+=`;background:${bg};color:${col};border:1px solid ${bd}`;
  el.textContent=msg;
}
function renderCfConfig(){renderEmailList();}
...
function processCSV(text, filename) {
  ...
  const iCorres  =colIdx(['correspondant','corresp','contact','responsable','interlocuteur','interne']);
...
function renderCsvDataTable(){
  ...
  tbody.innerHTML=csvData.slice(0,50).map(d=>{const key=d.t+'|'+d.dl,dlD=pDate(d.dl),days=dUntil(dlD),st=getSt(days),isTrans=transmittedSet.has(key);return`<tr style="${isTrans?'opacity:0.55':''}"><td><span class="status-dot ${st}"></span>${isTrans?' <span style="font-size:10px;color:var(--green)">✅</span>':''}</td><td class="td-title">${d.t}</td><td><span class="entity-pill" style="color:${eC(d.e)};background:${eCb(d.e)}">${d.e}</span></td><td style="font-size:12px;color:var(--text3)">${d.p}</td><td style="font-size:12px;color:var(--text3)">${d.c||'—'}</td><td style="font-family:'JetBrains Mono',monospace;font-size:12px">${fmtS(d.a)}</td><td style="font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:600">${fmtS(d.dl)}</td><td><span class="badge ${st}">${dLabel(days,st)}</span></td></tr>`;}).join('')+(csvData.length>50?`<tr><td colspan="7" style="text-align:center;color:var(--text3);font-size:12px;padding:12px">… et ${csvData.length-50} autres</td></tr>`:'');
}
...
// ══════════════════════════════════════════════════════
// ✉️ Cloud Function (voir functions/index.js)
// ══════════════════════════════════════════════════════
// ── Données ───────────────────────────────────────────
...
function buildData(){
  ...
  ALL=raw.map((d,i)=>{
    ...
    return{id:i,key,title:d.t,arrete:d.a||'—',deadline:d.dl,period:d.p,entity:d.e,correspondant:d.c||'—',dlD,days,st,custom:i>=csvData.length,transmitted};
...
function checkAndTriggerAlerts(){
  ...
  if(urgentDocs.length>0){playNotifSound();sendBrowserPushNotif(urgentDocs);// ✉️ Emails gérés par Cloud Function (7h00 GMT — voir functions/index.js)}
}
...
// ── Cartes ────────────────────────────────────────────
function makeCard(d){const dl=dLabel(d.days,d.st);return`<div class="deadline-card${d.transmitted?' card-transmitted':''}" onclick="openDetail(${d.id})"><div class="card-top-bar ${d.st}"></div><div class="card-body"><div class="card-entity-row"><span class="entity-pill" style="color:${eC(d.entity)};background:${eCb(d.entity)}">${d.entity}</span><span class="period-pill">${d.period}</span>${d.transmitted?'<span class="card-transmitted-tag">✅ Transmis</span>':''}</div><div class="card-title">${d.title}</div><div class="card-dates"><div class="date-block"><div class="date-lbl">Date d\'arrêté</div><div class="date-val">${fmtS(d.arrete)}</div></div><div class="date-block"><div class="date-lbl">Date limite</div><div class="date-val">${fmtS(d.deadline)}</div></div>${d.correspondant&&d.correspondant!=='—'?`<div class="date-block" style="grid-column:1/-1"><div class="date-lbl">👤 Correspondant interne</div><div class="date-val" style="color:var(--blue);font-weight:600">${d.correspondant}</div></div>`:''}</div><div class="card-footer-bar"><div class="countdown ${d.st}">${d.st==='urgent'?'🔴':d.st==='warn'?'🟡':d.st==='ok'?'🟢':'⚫'} ${dl}</div>${transmitBtn(d)}</div></div></div>`;}
...
function renderTable(){
  ...
  data.sort((a,b)=>{let va,vb;if(sortKey==='status'){va=stMap[a.st]??9;vb=stMap[b.st]??9;}else if(sortKey==='title'){va=a.title;vb=b.title;}else if(sortKey==='entity'){va=a.entity;vb=b.entity;}else if(sortKey==='period'){va=a.period;vb=b.period;}else if(sortKey==='correspondant'){va=a.correspondant||'';vb=b.correspondant||'';}else if(sortKey==='arrete'){va=a.arrete;vb=b.arrete;}else if(sortKey==='deadline'){va=a.dlD||new Date(0);vb=b.dlD||new Date(0);}else if(sortKey==='days'){va=a.days??9999;vb=b.days??9999;}else return 0;return(typeof va==='string'?va.localeCompare(vb):va-vb)*sortDir;});
  document.getElementById('tbl-count').textContent=data.length+' document'+(data.length>1?'s':'');
  document.getElementById('tbl-body').innerHTML=ALL.length===0?`<tr><td colspan="8" style="text-align:center;padding:40px;color:var(--text3)">📂 Aucune donnée — Importez un Excel dans <strong>Paramètres</strong></td></tr>`:data.map(d=>`<tr onclick="openDetail(${d.id})" style="cursor:pointer${d.transmitted?';opacity:0.65':''}"><td><span class="status-dot ${d.st}"></span>${d.transmitted?' <span style="font-size:10px;color:var(--green)">✅</span>':''}</td><td class="td-title">${d.title}</td><td><span class="entity-pill" style="color:${eC(d.entity)};background:${eCb(d.entity)}">${d.entity}</span></td><td style="font-size:12px;color:var(--text3)">${d.period}</td><td style="font-size:12px;color:var(--text3)">${d.correspondant||'—'}</td><td style="font-family:'JetBrains Mono',monospace;font-size:12px">${fmtS(d.arrete)}</td><td style="font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:600">${fmtS(d.deadline)}</td><td><span class="badge ${d.st}">${dLabel(d.days,d.st)}</span></td><td onclick="event.stopPropagation()">${transmitBtn(d,true)}</td></tr>`).join('');
}
...
function showEntityDetail(en){const items=ALL.filter(d=>d.entity===en).sort((a,b)=>a.dlD&&b.dlD?a.dlD-b.dlD:0);const sec=document.getElementById('entity-detail-section');sec.style.display='';document.getElementById('entity-detail-header').innerHTML='Toutes les échéances — '+en+' ('+items.length+' total)';document.getElementById('entity-detail-body').innerHTML=items.map(d=>`<tr onclick="openDetail(${d.id})" style="cursor:pointer${d.transmitted?';opacity:0.65':''}"><td><span class="status-dot ${d.st}"></span>${d.transmitted?' <span style="font-size:10px;color:var(--green)">✅</span>':''}</td><td class="td-title">${d.title}</td><td style="font-size:12px;color:var(--text3)">${d.period}</td><td style="font-size:12px;color:var(--text3)">${d.correspondant||'—'}</td><td style="font-family:'JetBrains Mono',monospace;font-size:12px">${fmtS(d.arrete)}</td><td style="font-family:'JetBrains Mono',monospace;font-size:12px;font-weight:600">${fmtS(d.deadline)}</td><td><span class="badge ${d.st}">${dLabel(d.days,d.st)}</span></td><td onclick="event.stopPropagation()">${transmitBtn(d,true)}</td></tr>`).join('');sec.scrollIntoView({behavior:'smooth',block:'start'});}
...
function renderSettings(){
  ...
  renderCustomList();renderEmailList();renderCsvDataTable();renderCfConfig();
}
...
function renderCustomList(){const tbody=document.getElementById('custom-tbody'),emp=document.getElementById('custom-empty');if(customDL.length===0){tbody.innerHTML='';emp.style.display='';return;}emp.style.display='none';tbody.innerHTML=customDL.map((d,i)=>`<tr><td class="td-title">${d.t}</td><td><span class="entity-pill" style="color:${eC(d.e)};background:${eCb(d.e)}">${d.e}</span></td><td style="font-size:12px;color:var(--text3)">${d.c||'—'}</td><td style="font-family:'JetBrains Mono',monospace;font-size:12px">${fmtS(d.dl)}</td><td style="font-size:12px;color:var(--text3)">${d.p}</td><td><button class="btn-delete" onclick="delCustom(${i})">Supprimer</button></td></tr>`).join('');}
...
function addFromModal(){const t=document.getElementById('am-title').value.trim(),dl=document.getElementById('am-deadline').value;if(!t||!dl){showToast('⚠️','Titre et date limite requis');return;}customDL.push({t,a:document.getElementById('am-arrete').value||'—',dl,p:document.getElementById('am-period').value,e:document.getElementById('am-entity').value,c:document.getElementById('am-corres').value.trim()});saveToFirestore();buildData();updateKPIs();closeOverlay('add-overlay');document.getElementById('am-title').value=document.getElementById('am-arrete').value=document.getElementById('am-deadline').value=document.getElementById('am-corres').value='';showToast('✓','"'+t+'" ajouté');}
function addFromForm(){const t=document.getElementById('f-title').value.trim(),dl=document.getElementById('f-deadline').value;if(!t||!dl){showToast('⚠️','Titre et date limite requis');return;}customDL.push({t,a:document.getElementById('f-arrete').value||'—',dl,p:document.getElementById('f-period').value,e:document.getElementById('f-entity').value,c:document.getElementById('f-corres').value.trim()});saveToFirestore();buildData();updateKPIs();renderCustomList();document.getElementById('f-title').value=document.getElementById('f-arrete').value=document.getElementById('f-deadline').value=document.getElementById('f-corres').value='';showToast('✓','"'+t+'" ajouté');}
function openDetail(id){
  ...
  const corresVal=d.correspondant||'—';
  document.getElementById('dm-corres').textContent=corresVal;
  document.getElementById('dm-corres-wrap').style.display=(corresVal==='—'||!corresVal)?'none':'';
