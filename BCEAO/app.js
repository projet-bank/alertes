...
function buildData(){
  ...
  ALL=raw.map((d,i)=>{
    ...
    const nextD=nextNotifDate(arrStr,d.p,d.jours);
    const days=dUntil(nextD);
    const key=d.t+'|'+(arrStr||d.t);
    const transmitted=transmittedSet.has(key);
    let dlD=nextD,finalDays=days,st;
    if(transmitted){
      // Toujours avancer d'une période complète depuis la prochaine date calculée
      const daysP=periodDays(d.p,d.jours);
      let next2=nextD?new Date(nextD.getTime()+daysP*86400000):null;
      if(next2){
        // Si next2 est encore dans le passé (cas rare), continuer d'avancer
        let it=0;
    ...
    return{id:i,key,title:d.t,arrete:arrStr||'—',deadline:dlStr,period:d.p,jours:d.jours||periodDays(d.p),entity:d.e,dlD,days:finalDays,st,custom:i>=csvData.length,transmitted:transmittedSet.has(key)};
...
function transmitBtn(d,small=false){
  ...
  if(d.transmitted)return`<span class="transmitted-badge">✅ Transmis <button class="btn-unmark" onclick="event.stopPropagation();unmarkTransmittedById('${k}')" title="Annuler la transmission">↩</button></span>`;
...
function updateDocPeriod(docId, newPeriod){
  ...
  const defaultJ=periodDays(newPeriod);
  if(d.custom){const ci=docId-csvData.length;if(customDL[ci]){customDL[ci].p=newPeriod;customDL[ci].jours=defaultJ;}}
  else{if(csvData[docId]){csvData[docId].p=newPeriod;csvData[docId].jours=defaultJ;}}
  saveToFirestore();buildData();updateKPIs();renderTable();showToast('✓','Périodicité : '+newPeriod+' ('+defaultJ+' j)');
}

// Mise à jour de la durée en jours d'un document directement depuis le tableau
function updateDocJours(docId, newJours){
  const j=parseInt(newJours);if(!j||j<1)return;
  const d=ALL.find(x=>x.id===docId);if(!d)return;
  // Mettre à jour la source de données
  if(d.custom){const ci=docId-csvData.length;if(customDL[ci])customDL[ci].jours=j;}
  else{if(csvData[docId])csvData[docId].jours=j;}
  // Mettre à jour directement dans ALL pour que renderTable ne réinitialise pas la valeur
  d.jours=j;
  // Recalculer la prochaine date pour ce document
  const nextD=nextNotifDate(d.arrete,d.period,j);
  d.dlD=nextD;
  d.days=dUntil(nextD);
  d.st=d.transmitted?'past':getSt(d.days);
  d.deadline=nextD?nextD.toISOString().slice(0,10):'—';
  saveToFirestore();updateKPIs();renderTable();showToast('✓','Durée mise à jour : '+j+' jours');
}
